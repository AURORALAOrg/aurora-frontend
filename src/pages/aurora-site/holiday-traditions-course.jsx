...
import { holidayTraditionsCourse } from "@/components/cultural-courses/holiday-traditions";
import LessonComponent from "@/components/cultural-courses/holiday-traditions/LessonComponent";

const HolidayTraditionsCoursePage = () => {
  ...
  const handleNextLesson = () => {
    const lastIndex = holidayTraditionsCourse.lessons.length - 1;
    if (currentLesson === -1) {
      setCurrentLesson(0);
      return;
    }
  ...
  const getProgressPercentage = () => {
    const total = holidayTraditionsCourse.lessons.length || 1;
        ...
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-8 mb-8">
            ...
            <p className="text-neutral-2 mb-6">
              Congratulations! You have successfully completed the Holiday
              Traditions course.
            </p>
            <div className="bg-dark-blue-5 rounded-lg p-6 mb-6">
              ...
              <ul className="text-left space-y-2 text-neutral-2">
                {holidayTraditionsCourse.courseOverview.objectives.map(
                  (objective, index) => (
    ...
    <div className="min-h-screen bg-[#111827] text-neutral-1">
      ...
      <div className="bg-dark-blue-5 border-b border-[#1f2937] p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-2xl font-bold text-neutral-1 mb-1">
            {holidayTraditionsCourse.title}
          </h1>
          <p className="text-neutral-2 mb-4">
            {holidayTraditionsCourse.description}
          </p>
          <div className="flex flex-wrap gap-3">
            <div className="bg-dark-blue-4 px-3 py-1 rounded text-xs text-neutral-2">
              ...
              {holidayTraditionsCourse.level}
            </div>
            <div className="bg-dark-blue-4 px-3 py-1 rounded text-xs text-neutral-2">
              <span className="font-medium text-neutral-1">
                Estimated time:
              </span>{" "}
              {holidayTraditionsCourse.estimatedTime}
            </div>
            <div className="bg-dark-blue-4 px-3 py-1 rounded text-xs text-neutral-2">
              <span className="font-medium text-neutral-1">Lessons:</span>{" "}
              {holidayTraditionsCourse.lessons.length}
            </div>
            <div className="bg-dark-blue-4 px-3 py-1 rounded text-xs text-neutral-2">
              <span className="font-medium text-neutral-1">Exercises:</span>{" "}
              {holidayTraditionsCourse.totalExercises}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center">
                <h2 className="font-semibold text-neutral-1">
                  {currentLesson === -1
                    ? "Course Overview"
                    : holidayTraditionsCourse.lessons[currentLesson]
                        .title}
                </h2>
        ...
        {currentLesson === -1 ? (
          <div className="bg-dark-blue-5 border border-[#1f2937] rounded-lg p-6 mb-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-neutral-2">
                  {holidayTraditionsCourse.courseOverview.skills.map(
                    (skill, index) => (
                      <li key={index} className="flex items-start">
                        <span className="text-light-blue-1 mr-2">•</span>{" "}
                        {skill}
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-neutral-2">
                  {holidayTraditionsCourse.courseOverview.prerequisites.map(
                    (prereq, index) => (
                      <li key={index} className="flex items-start">
                        <span className="text-light-blue-1 mr-2">•</span>{" "}
                        {prereq}
          ...
          <LessonComponent
            lessonData={
              holidayTraditionsCourse.lessons[currentLesson].data
            }
            onComplete={handleNextLesson}
            onNext={handleNextLesson}
            onPrevious={handlePreviousLesson}
            lessonNumber={currentLesson + 1}
            totalLessons={holidayTraditionsCourse.lessons.length}
          />
        )}
      </div>
    </div>
...
export default HolidayTraditionsCoursePage;
