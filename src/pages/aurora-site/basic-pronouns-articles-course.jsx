...
import basicPronounsArticlesCourse from '@/components/grammar-courses/basic-pronouns-articles';
import LessonComponent from '@/components/grammar-courses/basic-pronouns-articles/LessonComponent';

const BasicPronounsArticlesCoursePage = () => {
  ...
  const handleNextLesson = () => {
    if (currentLesson < basicPronounsArticlesCourse.lessons.length - 1) {
  ...
  const getProgressPercentage = () => {
    return Math.round((completedLessons.length / basicPronounsArticlesCourse.lessons.length) * 100);
  };
        ...
        <div className="max-w-4xl mx-auto text-center">
          <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-8 mb-8">
            ...
            <p className="text-neutral-2 mb-6">
              You completed the Basic Pronouns & Articles course!
            </p>
            <div className="bg-dark-blue-5 rounded-lg p-6 mb-6">
              <h2 className="text-xl font-semibold text-neutral-1 mb-4">You learned:</h2>
              <ul className="text-left space-y-2 text-neutral-2">
                {basicPronounsArticlesCourse.courseOverview.objectives.map((objective, index) => (
    ...
    <div className="min-h-screen bg-[#111827] text-neutral-1">
      <div className="bg-dark-blue-5 border-b border-[#1f2937] p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl font-bold mb-2">{basicPronounsArticlesCourse.title}</h1>
          <p className="text-neutral-2 mb-4">{basicPronounsArticlesCourse.description}</p>
          
          <div className="flex items-center space-x-4 text-sm text-neutral-2">
            <div className="flex items-center">
              <BookOpen className="w-4 h-4 mr-2" />
              {basicPronounsArticlesCourse.lessons.length} lessons
            </div>
            <div className="flex items-center">
            ...
            <div className="flex items-center">
              ...
              {basicPronounsArticlesCourse.estimatedTime}
            </div>
          </div>
        </div>
      </div>
      <div className="bg-dark-blue-4 border-b border-[#1f2937] p-4">
        <div className="max-w-4xl mx-auto">
          <div className="flex space-x-2 overflow-x-auto">
            {basicPronounsArticlesCourse.lessons.map((lesson, index) => (
              <button
                key={lesson.id}
      ...
      <div className="p-6">
        <div className="max-w-4xl mx-auto">
          <LessonComponent
            lessonData={basicPronounsArticlesCourse.lessons[currentLesson].data}
            lessonNumber={currentLesson + 1}
            totalLessons={basicPronounsArticlesCourse.lessons.length}
        ...
        <div className="space-y-4">
          <div>
            ...
            <ul className="text-xs text-neutral-3 space-y-1">
              {basicPronounsArticlesCourse.courseOverview.objectives.slice(0, 3).map((objective, index) => (
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {basicPronounsArticlesCourse.courseOverview.skills.map((skill, index) => (
          ...
          <div>
            ...
            <div className="text-xs text-neutral-3 space-y-1">
              <div>• {basicPronounsArticlesCourse.totalExercises} exercises</div>
              <div>• {basicPronounsArticlesCourse.totalQuestions} questions</div>
              <div>• {basicPronounsArticlesCourse.lessons.length} lessons</div>
            </div>
          </div>
        </div>
      </div>
...
export default BasicPronounsArticlesCoursePage;